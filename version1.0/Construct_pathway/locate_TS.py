...
def run_GSM(c):
    ...
    if c["input_type"] == 0:
        ...
        if c["pre-opt"] is True:
            ...
            for lp,product_xyz in enumerate(c["products"]):
                ...
                Energy,opted_geo,finish = xtb_geo_opt(product_xyz,charge=c["charge"],unpair=c["unpair"],namespace='product_{}'.format(lp),\
                ...
                if oinchi[:14] == ninchi[:14]:
                    ...
                    new_G = opt_geo(PG,adj_mat,PE,ff='Ghemical',step=1000)
                    ...
                    Energy,reactant_opt_geo,finish = xtb_geo_opt(tmpxyz,charge=c["charge"],unpair=c["unpair"],namespace='reactant',workdir=c["output_path"]+'/xTB-folder',\
                                                                 level='normal',output_xyz=reactant_opt,cleanup=False)
                    if not finish:
                        continue
    ...
    # submit jobs
    substring="python {}/utilities/job_submit.py -f '*.submit' -sched {}".format('/'.join(os.path.abspath(__file__).split('/')[:-2]),c["sched"])
    ...
    if c["batch"] == 'pbs':
        ...
        print("\t running {} GSM-xTB jobs...".format(len(output.split())/4))
    ...
    for output_folder in output_list:
        ...
        if 'Finished GSM!\n' in lines:
            ...
            if E_TS < 1000:
                ...
                substring = substring.format('/'.join(os.path.abspath(__file__).split('/')[:-2]),TS_folder,pname,TS_folder,pname,c["charge"],c["multiplicity"],c["functional"],c["basis"],pname)
                os.system(substring)
    ...
    substring = substring.format('/'.join(os.path.abspath(__file__).split('/')[:-2]),TSgjf_list,TS_folder,c["parallel"],c["procs"],c["njobs2"],c["ppn"],c["queue2"],c["memory"],c["sched"],c["wt2"]) 
    os.system(substring)
    ...
    if c["batch"] == 'pbs':
        ...
        print("\t running {} Gaussian TS_opt jobs...".format(len(output.split())/4))
    monitor_jobs(output.split())
    os.system('rm *.submit')
    ...
    for TSgjf in TSgjf_list:
        ...
        if imag_flag and finish_flag:
            ...
            os.system(command.format('/'.join(os.path.abspath(__file__).split('/')[:-2]),TSout,IRC_folder,pname,N_dict[pname]))
            ...
            substring = substring.format('/'.join(os.path.abspath(__file__).split('/')[:-2]),IRC_folder,pname,IRC_folder,pname,c["charge"],c["multiplicity"],\
                                         c["functional"],c["basis"],c["irc-image"],c["stepsize"],pname)
            os.system(substring)
    ...
    substring = substring.format('/'.join(os.path.abspath(__file__).split('/')[:-2]),IRCgjf_list,IRC_folder,c["parallel"],c["procs"],c["njobs2"],c["ppn"],c["queue2"],c["memory"],c["sched"],c["wt2"]) 
    os.system(substring)
    ...
    if c["batch"] == 'pbs':
        ...
        print("\t running {} Gaussian IRC jobs...".format(len(output.split())/4))
    ...
    for IRCgjf in IRCgjf_list:
        ...
        if finish_flag:
            ...
            os.system(command.format('/'.join(os.path.abspath(__file__).split('/')[:-2]),IRCout,IRC_folder,pname,N_dict[pname]))
...
def parse_configuration(args):
    ...
    keywords = ["input_type","reactant_xyz","product_path","input_xyz","output_path","e_dict","charge","unpair","multiplicity",\
                "pre-opt","mode","nimage","add-tor","conv-tor","batch","sched","wt","njobs","queue",\
...
def check_queue():
    ...
    if not hasattr(check_queue, "user"):
        ...
        # Get user name
        check_queue.user = subprocess.check_output("echo ${USER}", shell=True).strip("\r\n")

        # Get batch system being used
        squeue_tmp = subprocess.Popen(['which', 'squeue'], stdout=subprocess.PIPE,stderr=subprocess.STDOUT).communicate()[0].strip("\r\n")
        qstat_tmp = subprocess.Popen(['which', 'qstat'], stdout=subprocess.PIPE,stderr=subprocess.STDOUT).communicate()[0].strip("\r\n")
    ...
    if check_queue.sched == "slurm":
        ...
        output = subprocess.check_output("squeue -l", shell=True)
    ...
    # Get running and pending jobs using the pbs scheduler
        ...
        output = subprocess.check_output("qstat -f", shell=True)
