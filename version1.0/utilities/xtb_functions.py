...
def xtb_energy(xyz_file,charge=0,unpair=0,niter=100,accuracy=1.0,namespace='xTB',workdir='.',method='sp'):
...
# 
################################################################################################################################################
def xtb_geo_opt(xyz_file,charge=0,unpair=0,niter=100,accuracy=1.0,namespace='xTB',workdir='.',level='normal',output_xyz=None,cleanup=False):
    current_dir = os.getcwd()
    # change to xTB working directory
    os.chdir(workdir)
    ...
    #output = output.decode('utf-8')
    Energy = 0.0
    with open("{}_xTB-opt.txt".format(namespace),"w") as g:
        g.write(output)
