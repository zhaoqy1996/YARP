            ...
            with open('GSM.{}.submit'.format(n),'w') as f:                                                                 
                ...
                for product in input_products:
                    ...
                    with open("{}/xtb_lot.txt".format(output_folder),"w") as g:
                        ...
                        command_line = 'python {}/external-software/pyGSM/pygsm/wrappers/main.py -xyzfile {} -mode DE_GSM -package xTB -lot_inp_file {} ' +\
                                       '-CONV_TOL {} -ADD_NODE_TOL {} -num_nodes {} -charge {} -multiplicity {} > log &\n'

                        command_line = command_line.format('/'.join(os.path.abspath(__file__).split('/')[:-2]),product,output_folder+'/xtb_lot.txt',conv_tor,add_tor,Nimage,charge,multiplicity)
