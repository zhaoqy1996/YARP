...
class xTB(Lot):
    ...
    def run(self,geom,multiplicity):
        ...
        try:
            ...
            xTBscr = 'tempxTBrun'
            runscr = '/tmp/'+user+'/'+xTBscr

        # command line
        substring="{} -c {} -u {} -a {} --iterations {} --{} --namespace '{}' --norestart {}.xyz"
        sub_cmd  =substring.format(path2xTB,config["charge"],config["spin"],config["accuracy"],config["max_iter"],config["calc_type"],config["namespace"],tempfilename)
