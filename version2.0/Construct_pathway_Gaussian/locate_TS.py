...
def run_YARP_Gaussian(c):
    ...
    if c["level"].lower() == 'xtb': os.system('cp {}/external-software/gau_xtb/* {}'.format('/'.join(os.path.abspath(__file__).split('/')[:-3]),c["output_path"]))
    if c["level"].lower() == 'ani':
        print("Current version doesn't support ani/aimnet yet!")
        quit()
    ...
    DFT_finish,tmp = check_DFT_energy(reactant_smiles,E_dict,conf_path,functional=c["functional"],basis=c["basis"],dispersion=c["dispersion"],solvation=c["solvation"],config=energy_config)
    ...
    output_list = submit_GSM(final_products,c["output_path"],int(c["low_njobs"]),c["pygsm_path"],level=c["level"],procs=c["low_procs"],Wt=c["low_wt"],sched=c["sched"],queue=c["low_queue"],\
                             charge=qt,unpair=unpair,Nimage=int(c["nimage"]),conv_tor=c["conv-tor"],add_tor=c["add-tor"],relax_end=c["relax_end"])
    ...
    for output_folder in output_list:
        ...
        if pname:
            ...
            if c["level"].lower() == 'xtb':
                ...
                substring = "python {}/utilities/xyz_to_Gaussian_xtb.py {}/{}-TS.xyz -o {}/{}/{}-TS.gjf -q {} -m {} -c {} -cs 1" + \
                            " -ty \"P OPT=(TS, CALCALL, NOEIGEN, NOMICRO, maxcycles=200) external='./../xtb.sh' \" -t \"{} TS\" "
                substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),ini_folder,pname,c["output_path"],pname,pname,qt,unpair+1,pname,pname)
                os.system(substring)
                ...
                substring = "python {}/utilities/xyz_to_Gaussian.py {}/{}-TS.xyz -o {}/{}/{}-TS.gjf -q {} -m {} -c False" + \
                            " -ty \"{}/{} OPT=(TS, CALCALL, NOEIGEN, maxcycles=100) \" -t \"{} TS\" "
                substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),ini_folder,pname,c["output_path"],pname,pname,qt,unpair+1,c["level"].split('/')[0],c["level"].split('/')[1],pname)
                os.system(substring)
    ...
    for TSinp in TSinp_list:
        ...
        if imag_flag and finish_flag:
            ...
            if c["level"].lower() == 'xtb':
                ...
                substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),c["output_path"],pname,pname,c["output_path"],pname,pname,qt,unpair+1,c["level"].split('/')[0],\
                                             c["level"].split('/')[1],c["irc-image"],c["stepsize"],pname)
                os.system(substring)
    ...
    TSinp_list,low_IRC_dict = read_IRC_outputs(c["output_path"],IRCinp_list,adj_dict,hash_dict,N_dict,reaction_dict,select=True,folder_name='low-IRC-result',dg_thresh=c['dg_thresh'])
    ...
    for TSinp in TSinp_list:
        ...
        if imag_flag and finish_flag:
            ...
            if c["solvation"] is not None: DFTlevel += ' SCRF=({},solvent={})'.format(c["solvation"].split('/')[0],c["solvation"].split('/')[1])
            
            substring = "python {}/utilities/xyz_to_Gaussian.py {}/{}/{}-TS.xyz -o {}/{}-TS.gjf -q {} -m {} -c False -ty \"{} OPT=(TS, CALCALL, NOEIGEN, maxcycles=100) Freq \" -t \"{} TS\" "
            substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),c["output_path"],pname,pname,TS_folder,pname,qt,unpair+1,DFTlevel,pname)
            os.system(substring)
    ...
    N_grad,reaction_dict,TS_DFT_inp = channel_classification(output_path=c["output_path"],initial_xyz=input_folder,inp_list=TS_DFT_inp,low_IRC_dict=low_IRC_dict,N_dict=N_dict,adj_dict=adj_dict,F_dict=F_dict,reaction_dict=reaction_dict,\
                                                             append=False,thresh=0.5,model=c["irc-model"],dg_thresh=c["dg_thresh"])
    ...
    if c["dft-irc"]:
        ...
        for TSinp in TS_DFT_inp:
            ...
            if imag_flag and finish_flag:
                ...
                substring = "python {}/utilities/xyz_to_Gaussian.py {}/{}-TS.xyz -o {}/{}-IRC.gjf -q {} -m {} -c False -ty \"{} IRC=(LQA,recorrect=never,CalcFC,maxpoints={},StepSize={},maxcyc=100,Report=Cartesians)\" -t \"{} IRC\" "
                           #" -ty \"{} IRC=(CALCALL,maxpoints={},StepSize={},maxcyc=100,Report=Cartesians)\" -t \"{} IRC\" "
                substring = substring.format('/'.join(os.getcwd().split('/')[:-1]),TS_folder,pname,IRC_folder,pname,qt,unpair+1,DFTlevel,c["irc-image"],c["stepsize"],pname)
                os.system(substring)
        ...
        reaction_dict = read_IRC_outputs(c["output_path"],IRCinp_list,adj_dict,hash_dict,N_dict,reaction_dict,F_dict,TSEne_dict,append=False,select=False,folder_name='IRC-result')
...
def parse_configuration(args):
    ...
    keywords = ["input_type","reactant_dict","reaction_dict","input_xyz","output_path","input_react","pygsm_path","e_dict","ff","apply_tcit","tcit_result","hf_cut","charge","unpair","batch","sched","restart_step",\
                "select_conf","dft-irc","memory","irc-image","stepsize","irc-model","dg_thresh","compute_product","level","nimage","add-tor","conv-tor","relax_end","low_wt","low_njobs","low_queue","low_procs",\
                "c_wt","c_njobs","c_nprocs","n_max","c_queue","add-joint","c_path","functional","basis","dispersion","solvation","high_wt","ppn","high_procs","high_njobs","high_queue","parallel"]
